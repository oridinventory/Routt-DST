...
function(require, lang, array, aspect, Deferred, cookie, json, topic, dojoScript, esriNS,
  ...
  var mo = {
      ...
      dojoScript.get(httpsTokenUrl, {
        ...
        if(response.token){
          ...
          esriNS.id.getCredential(httpsSharingUrl).then(lang.hitch(this, function(credential){
            ...
            var corsListIndex = esriUrlUtils.isTrustedServer(credential.server, true);
            if (corsListIndex > -1) {
              io.corsEnabledServers && io.corsEnabledServers.splice(corsListIndex, 1);
            }

            // ADD a new entry for this portal in CORS list.
            // io.corsEnabledServers.push({
    ...
    addWithCredentialDomain: function(domain){
      ...
      if(domain && typeof domain === 'string'){
        var io = jimuUtils.getEsriConfigRequestObject();
        var corsEnabledServers = io.corsEnabledServers || [];
        ...
        var corsListIndex = esriUrlUtils.isTrustedServer(serverWithProtocol, true);
        if (corsListIndex > -1) {
    ...
    _pushCorsEnabledServerInfo: function(serverInfo){
      ...
      if(typeof serverInfo === "object" && typeof serverInfo.host === "string"){
        ...
        if(serverInfo.withCredentials){
          if(IO.webTierAuthServers){
            if(IO.webTierAuthServers.indexOf(serverInfo.host) < 0){
              IO.webTierAuthServers.push(serverInfo.host);
            }
          }
        }
      }
     ...
     *
     * The signOutAll method of 3D will never be involevd as there is no sign-out operation from app.
     */
    ...
    startup: function(){
      ...
      if(this.isInConfigOrPreviewWindow()){
        ...
        if(builderWindow){
          ...
          if(builderIM){
            //sync from builder's IdentityManager
            IdentityManager.initialize(builderIM.toJson());

            var currentIO = jimuUtils.getEsriConfigRequestObject();
            var builderIO = builderWindow.esriConfig.defaults.io;
            if(!currentIO.trustedServers) {
              currentIO.trustedServers = [];
            }
            array.forEach(builderIO.corsEnabledServers || [], lang.hitch(this, function(server) {
              if(server && server.host && server.withCredentials) {
                currentIO.trustedServers.push(server.host);
              }
            }));
          }
        }
