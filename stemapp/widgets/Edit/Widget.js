...
define([
  ...
  function(declare, lang, array, html, esriBundle, on, query, Json, Deferred, aspect, all,
    _WidgetsInTemplateMixin, BaseWidget, MapManager, PanelManager, LayerInfos, LoadingIndicator, JimuPopup,
    /*jimuUtils,*/ portalUrlUtils, /*portalUtils,*/ SelectionManager, Role, Editor, editingUtil, Popup, TemplatePicker,
    Extent, Point, SimpleMarkerSymbol, rendererJsonUtils, SimpleRenderer, Graphic, domUtils, EsriQuery, QueryTask,
    Button, editUtils, FilterEditor, RelatedRecordsEditor) {
    var Clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      _newAddedLayersWhileEditWidgetIsClosed: null,
      ...
      onOpen: function() {
        ...
        if(this._newAddedLayersWhileEditWidgetIsClosed && this._newAddedLayersWhileEditWidgetIsClosed.length > 0) {
          // this._newAddedLayersWhileEditWidgetIsClosed arrary will be emptied while refresh.
          this._refresh();
          return;
        }
        ...
        if(this.editor && this._settingsParam) {
      ...
      _getDefaultLayerInfoById: function(layerId) {
        ...
        layerInfo.allowCreate = true;
        layerInfo.allowDelete = true;
      ...
      // Due to the code history compatible reason, the values of this._configEditor.layerInfos can be:
      // 1, this._configEditor.layerInfos === [] :
      //    means the edit widget's setting page has not been opened yet, using the defautl setting.
      // 2, this._configEditor.layerInfos === null or undefined :
      //    means the edit was set in the setting page and no layers were checked.
      // 3, this._configEditor.layerInfos.length > 0 :
      //    means the edit was set in the setting page and at least one layer is checked.
      _getLayerInfosInConfig: function() {
        var layerInfos;
        if(this._configEditor.honorSettingOfWebmap) {
          // honor webmap's setting
          layerInfos = this._getDefaultLayerInfos();
        } else if(!this._configEditor.layerInfos) {
      ...
      _getLayerInfoOrTableInfoParamById: function(layerId) {
        var layerOrTableInfoParam = null;
        array.some(this.layerInfosParam.concat(this.tableInfosParam), function(layerOrTableInfo) {
          if(layerOrTableInfo.featureLayer.id === layerId) {
            layerOrTableInfoParam = layerOrTableInfo;
            return true;
          }
        });
        return layerOrTableInfoParam;
      },
      ...
      _getLayerInfosParam: function() {
        ...
        array.forEach(layerInfos, function(layerInfo) {
          ...
          var layerObject = this.map.getLayer(layerId);
          if(layerObject &&
             //layerObject.visible && // will be filtered before pass to templatePicker
      ...
      _setTemplatePickerLayerObjectsParam: function(layerObjects) {
        if(layerObjects) {
          this._layerObjectsParaForTempaltePicker = layerObjects;
        }
      },

      _obtainTemplatePickerLayerObjectsParam: function(layerInfos) {
        this._layerObjectsParaForTempaltePicker = [];

        // reorder the layers.
        var layerInfosWithNewOrder = [];
        array.forEach(this._jimuLayerInfos.getLayerInfoArray(), function(jimuLayerInfo) {
          array.some(layerInfos, function(layerInfo) {
            if(layerInfo.featureLayer && jimuLayerInfo.id === layerInfo.featureLayer.id) {
              layerInfosWithNewOrder.push(layerInfo);
              return true;
            } else {
              return false;
            }
        ...
        array.forEach(layerInfosWithNewOrder, function(layerInfo) {
          if(!layerInfo.featureLayer) {
            return;
          }
          var jimuLayerInfo = this._jimuLayerInfos.getLayerInfoByTopLayerId(layerInfo.featureLayer.id);
          if(jimuLayerInfo &&
            jimuLayerInfo.isShowInMap() &&
            jimuLayerInfo.isInScale() &&
            //layerInfo.featureLayer.getEditCapabilities &&
            //layerInfo.featureLayer.getEditCapabilities().canCreate) {
            editUtils.getEditCapabilities(jimuLayerInfo.layerObject, layerInfo).canCreate) {
          ...
          if(!this._canCreateLayersAreAllInvisibleFlag &&
             this._isEditableLayerStore[layerInfo.featureLayer.id] &&
             //layerInfo.featureLayer.getEditCapabilities &&
             //layerInfo.featureLayer.getEditCapabilities().canCreate &&
             editUtils.getEditCapabilities(jimuLayerInfo.layerObject, layerInfo).canCreate &&
             !layerInfo.featureLayer.visible) {
      ...
      _getTemplatePicker: function(layerInfos) {
        var layerObjectsParaForTempaltePicker = this._obtainTemplatePickerLayerObjectsParam(layerInfos);
        ...
        var templatePicker = new TemplatePicker({
          featureLayers: layerObjectsParaForTempaltePicker,
          grouping: true,
          rows: "auto",
          columns: "auto",
      ...
      ****************************************/
      _addButtonToInspector: function() {
        ...
        if(this._configEditor.usingSaveButton) {
          ...
          attributeInspector.onAttributeChange = lang.hitch(attributeInspector,
            ...
            attributeInspector._currentFeature.attributes[fieldName] = newFieldValue;
          });
      ...
      _worksAfterCreate: function(settings) {
        ...
        this.ATILoading = new LoadingIndicator({
          hidden: true
        ...
        //working around for support 'attribute' button even though the selection ahead of the edit startup.
        //if(this.editor && this.editor.drawingToolbar && this.editor.drawingToolbar._updateUI) {
        //  this.editor.drawingToolbar._updateUI();
        //}
        var selectedFeature = null;
        var selectedLayer = null;
        var selectedFeatures = editingUtil.getSelection(this._settingsParam.layers);
        ...
        if(selectedLayer) {
          SelectionManager.getInstance().addFeaturesToSelection(selectedLayer, [selectedFeature]);
        }
      ...
      _worksAfterDestory: function() {
        ...
        // logic has been changed, release events after destory now.
      ...
      _bindEventsAfterCreate: function(settings) {
        ...
        */
        ...
        this._releaseEventArrayAfterDestory.push(handle);
        ...
        // having to use for control delete button in tool bar while selecte features by selecte tool.
        ...
        var templatePickerSelectionChangeHandler = on(this.editor.templatePicker,
          ...
          if(this.editor.templatePicker &&
            ...
            this._layerInfosChangedWhenEditingSessionFlag = false;
            // react the layerInfosChange when editing is finished.
            if(!this._isInEditingRelatedGraphicSession) {
              //does not react the layerInfosChange in the editing related graphic session;
              //otherwise will cause mistake.
              this._onLayerInfosChanged();
            }

          }
        }));
        this._releaseEventArrayAfterDestory.push(templatePickerSelectionChangeHandler);

        // bug fixes for drawing tool can't be changed if layer's geometry is not changed.
        handle = aspect.before(this.editor,
        ...
        // listen layer added or removed
        handle = on(this._jimuLayerInfos, 'layerInfosChanged', lang.hitch(this, this._onLayerAddedOrRemoved));
        this._releaseEventArrayAfterDestory.push(handle);
      },

      _releaseEventAfterDestory: function() {
        array.forEach(this._releaseEventArrayAfterDestory, function(handle) {
          handle.remove();
        }, this);
        this._releaseEventArrayAfterDestory = [];
      },
      ...
      _bindEventAfterOpen: function() {
        //listen layerInfos event.
        var handle = on(this._jimuLayerInfos,
                        'layerInfosIsShowInMapChanged',
                        lang.hitch(this, this._onLayerInfosChanged));
        this._releaseEventArrayAfterClose.push(handle);
        handle = on(this._jimuLayerInfos, 'layerInfosReorder', lang.hitch(this, this._onLayerInfosChanged));
        this._releaseEventArrayAfterClose.push(handle);
        handle = on(this.map, 'zoom-end', lang.hitch(this, this._onLayerInfosChanged));
        this._releaseEventArrayAfterClose.push(handle);
      },
      ...
      _onLayerAddedOrRemoved: function(newJimuLayerInfo, type) {
        // Due to the new added layer does not set in the custom setting, so 'onAddLayer' only respond for 'Honor settings in the web map‘ mode of edit weidget setting.
        var doesNotSetInSettingPage = this._configEditor.layerInfos && (this._configEditor.layerInfos.length === 0);
        var isUseDefaultStting = this._configEditor.honorSettingOfWebmap || doesNotSetInSettingPage;
        if(type === LayerInfos.ADDED && isUseDefaultStting) {
          var layerAlreadyExist = array.some(this._jimuLayerInfos.getLayerInfoArrayOfWebmap(), function(jimuLayerInfo) {
            if(jimuLayerInfo.id === newJimuLayerInfo.id) {
              return true;
            } else {
              return false;
            }
          ...
          if(!layerAlreadyExist) {
            // new added layer.
            newJimuLayerInfo.isEditable().then(lang.hitch(this, function(isEditable) {
              // for adding layers, has to refresh the edit widget.
              if(isEditable) {
                if(this.state === 'opened' || this.state === 'active') {
                  this._refresh();
                } else {
                  this._newAddedLayersWhileEditWidgetIsClosed.push(newJimuLayerInfo.id);
                }
              }
            }));
          }
        } else if(type === LayerInfos.REMOVED) {
          // for removing layers, can update edit widtet on real-time.
          this._onLayerInfosChanged();
        }
      },
      ...
      _onLayerInfosChanged: function() {
        /*
        var reorderedLayerObjects = [];
        // reorder the layers.
        array.forEach(this._jimuLayerInfos.getLayerInfoArray(), function(layerInfo) {
          array.some(this._settingsParam.layerInfos, function(layerInfoParam) {
            if(layerInfoParam.featureLayer && layerInfo.id === layerInfoParam.featureLayer.id) {
              reorderedLayerObjects.push(layerInfoParam.featureLayer);
              return true;
            } else {
              return false;
            }
        ...
        var newLayerObjects = array.filter(reorderedLayerObjects, function(layerObject) {
          var layerInfo = this._jimuLayerInfos.getLayerInfoByTopLayerId(layerObject.id);
          if(layerInfo &&
        ...
        this._layerObjectsParaForTempaltePicker = newLayerObjects;
        */

        if(this.editor.templatePicker && this.editor.templatePicker.getSelected()) {
          // ignore layerInfos changed when editing session.
          this._layerInfosChangedWhenEditingSessionFlag = true;
          return;
        }

        var newLayerObjects = [];
        if(this._isInEditingRelatedGraphicSession) {
          this._startEditingRelatedGraphic(this._temporaryDataForEditingRelatedGraphicSession.currentOperationData,
                                           this._temporaryDataForEditingRelatedGraphicSession.currentIntegrityField,
                                           true,
                                           true);
        } else {
          newLayerObjects = this._obtainTemplatePickerLayerObjectsParam(this._settingsParam.layerInfos);
          this._update(newLayerObjects);
        }
      ...
      _changeToServiceRenderer: function(settings) {
        ...
        array.forEach(settings.layerInfos, function(layerInfo) {
          ...
          var serviceDefJson = Json.parse(layerInfo.featureLayer._json);
          var serviceRendererJson = serviceDefJson.drawingInfo.renderer;
          var serviceRender = null;
          var temporaryRenderer = null;
          if (layerRenderer.declaredClass === "esri.renderer.HeatmapRenderer") {
            serviceRender = rendererJsonUtils.fromJson(serviceRendererJson);
            if(serviceRender.declaredClass === "esri.renderer.HeatmapRenderer") {
              temporaryRenderer = new SimpleRenderer(new SimpleMarkerSymbol({
                size: 5
              }));
            } else {
              temporaryRenderer = serviceRender;
            }
            layerInfo._layerRenderer = layerRenderer;
            this._layerInfoParamArrayUseForRervertRenderre.push(layerInfo);
            layerInfo.featureLayer.setRenderer(temporaryRenderer);
            layerInfo.featureLayer.redraw();
          }
        }, this);
      },
      ...
      _getEditCapabilitiesByFeature: function(feature) {
        var editCapabilities = {};
        var featureLayer = feature && feature.getLayer && feature.getLayer();
        // featureLayer can be a temetory graphic layer at here if the layer is 'add feature' only.
        if(featureLayer && featureLayer.isEditable && featureLayer.isEditable()) {
          var layerOrTableInfoParam = this._getLayerInfoOrTableInfoParamById(featureLayer.id);
          if(featureLayer) {
            editCapabilities = editUtils.getEditCapabilities(featureLayer, layerOrTableInfoParam, {feature: feature});
          }
        }
      ...
      _disableToAnswerEventForEditorATI: function() {
        ...
        // disable to answer onLayerSelctionClear for editor ATI.
        if(!this._mapInfoStorage.editorATIonLayerSelectionClear) {
          this._mapInfoStorage.editorATIonLayerSelectionClear =
            this.editor.attributeInspector.onLayerSelectionClear;
          this.editor.attributeInspector.onLayerSelectionClear = lang.hitch(this, function(){});
        }
      ...
      _enableToAnswerEventForEditorATI: function() {
        ...
        // enable to answer onSelctionClear for editor ATI.
        if(this._mapInfoStorage.editorATIonLayerSelectionClear) {
          this.editor.attributeInspector.onLayerSelectionClear =
            lang.hitch(this.editor.attributeInspector, this._mapInfoStorage.editorATIonLayerSelectionClear);
      ...
      _getTableInfosParam: function() {
        ...
        } else if(!this._configEditor.tableInfos) {
          // configured in setting page and no layers checked.
          tableInfos = [];
      ...
      _startEditingRelatedGraphic: function(operationData,
                                            integrityField,
                                            denyClearSelection,
                                            denySelecteDefaultTempalte) {

        this._stopEditingRelatedGraphic();
        // set _temporaryDataForEditingRelatedGraphicSession
        this._temporaryDataForEditingRelatedGraphicSession.currentOperationData = operationData;
        this._temporaryDataForEditingRelatedGraphicSession.currentIntegrityField = integrityField;

        this._isInEditingRelatedGraphicSession = true;
        var relatedLayerInfo = operationData.destJimuLayerInfo;
        var relatedLayerObject = operationData.destJimuLayerInfo.layerObject;

        if(!denyClearSelection) {
          // hide popup and clear selection.
          this.editPopup.hide();
          this.editor._clearSelection();
        }
        ...
        */
        var newLayerObjects = [];
        if(relatedLayerInfo.isShowInMap() && relatedLayerInfo.isInScale()) {
          newLayerObjects = [relatedLayerObject];
        }
        this._setTemplatePickerLayerObjectsParam(newLayerObjects);
        this._update(newLayerObjects);
        if(this._filterEditor) {
          this._filterEditor.selectLayerFilterByValue(relatedLayerObject.id);
          this._filterEditor.disableLayerFilter();
      ...
      _stopEditingRelatedGraphic: function() {
        ...
        // clear _temporaryDataForEditingRelatedGraphicSession
        this._temporaryDataForEditingRelatedGraphicSession = {};
      ...
      _changeMessageForTemplatePicker: function() {
        ...
        if(this.editor && this.editor.templatePicker.featureLayers.length === 0) {
      ...
      _refreshAttributeInspector: function() {
        ...
        if(this.editPopup.isShowing) {
          var currentFeature = this.editor &&
                               this.editor.attributeInspector &&
                               this.editor.attributeInspector._currentFeature;
          this._updateDeleteButtonInPopup(currentFeature);
        }
      },

      // delete, cut, reshape...
      _updateOperationalButtons: function(currentFeature) {
        this._updateDeleteButtonInPopup(currentFeature);
        this._updateGeometryOperationButtons(currentFeature);
      },

      // udate delete button.
      _updateDeleteButtonInPopup: function(currentFeature) {
        var editCapabilities = this._getEditCapabilitiesByFeature(currentFeature);
        if(editCapabilities.canDelete) {
          //this._enableDeleBtnInToolbar(); // will be updated by selection-complete
          this._enableDeleBtnInPopup();
        } else {
          //this._disableDeleteBtnInToolbar(); // will be updated by selection-complete
      ...
      _disableDeleteBtnInPopup: function() {
        query(".atiButton.atiDeleteButton", this.editPopup.domNode).style("display", "none");
      },

      _enableDeleBtnInPopup: function() {
        query(".atiButton.atiDeleteButton", this.editPopup.domNode).style("display", "block");
      },

      _updateGeometryOperationButtons: function(currentFeature) {
        var currentFeatureLayer = currentFeature &&
                                  currentFeature.getLayer &&
                                  currentFeature.getLayer();
        var editCapabilities = this._getEditCapabilitiesByFeature(currentFeature);
        if(!currentFeatureLayer || editCapabilities.canUpdateGeometry) {
          //if feature service is 'add feature only', currentFeature is a local temporary graphic,
          //edit will add it to client first, keep tools on toolbar for this situation.
      ...
      _disableGeometryOperationButtons: function() {
        if(this._configEditor.toolbarVisible) {
          query("[widgetid=btnFeatureCut]", this.editor.domNode).style("display", "none");
          query("[widgetid=btnFeatureUnion]", this.editor.domNode).style("display", "none");
          query("[widgetid=btnFeatureReshape]", this.editor.domNode).style("display", "none");
          if(this._configEditor.toolbarOptions.mergeVisible ||
             this._configEditor.toolbarOptions.cutVisible ||
             this._configEditor.toolbarOptions.reshapeVisible) {
            query("[widgetid^=dijit_ToolbarSeparator]", this.editor.domNode).last().style("display", "none");
          }
        }
      },
      ...
      _enableGeometryOperationButtons: function() {
        if(this._configEditor.toolbarVisible) {
          query("[widgetid=btnFeatureCut]", this.editor.domNode).style("display", "inline-block");
          query("[widgetid=btnFeatureUnion]", this.editor.domNode).style("display", "inline-block");
          query("[widgetid=btnFeatureReshape]", this.editor.domNode).style("display", "inline-block");
          if(this._configEditor.toolbarOptions.mergeVisible ||
             this._configEditor.toolbarOptions.cutVisible ||
             this._configEditor.toolbarOptions.reshapeVisible) {
            query("[widgetid^=dijit_ToolbarSeparator]", this.editor.domNode).last().style("display", "inline-block");
          }
        }
      },

      _updateDeleteButtonInToolbarBySelectionFeatures: function() {
        // canDeleteFeatures is ture if all features can be deleted,
        // else it is false.
        ...
        if(selectionFeatures.length === 0) {
          ...
          array.some(selectionFeatures, function(feature) {
            ...
            if(!featureLayer ||
               ...
               !editUtils.getEditCapabilities(featureLayer, layerOrTableInfoParam, {feature: feature}).canDelete
              ) {
              canDeleteFeatures = false;
              return true;
            }
            */
            ...
            var editCapabilities = this._getEditCapabilitiesByFeature(feature);
            if(!editCapabilities.canDelete) {
              canDeleteFeatures = false;
              return true;
            }
       ...
       * About events
       ************************/
      _update: function(layerObjects) {
        if(this.editor){
          if(layerObjects) {
            if(this._filterEditor) {
              this._filterEditor.update();
            }
            this.editor.templatePicker.featureLayers = layerObjects;
          }
          this.editor.templatePicker.clearSelection();
          this.editor.templatePicker.update(true);
          this._changeMessageForTemplatePicker();
      ...
      _onNextOfEditorATI: function(evt) {
        ...
        this._updateOperationalButtons(evt.feature);
      },

      _onLayerSelectionChange: function() {
        // use for control delete button in tool bar.
        // having to use for control delete button in tool bar while selecte features by selecte tool.
