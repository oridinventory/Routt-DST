  ...
  return declare([BaseWidget, Evented, _WidgetsInTemplateMixin], {
    ...
    updateProjectInfo: function (projectInfo) {
      ...
      if (this.config.projectSettings.projectLayer) {
        ...
        domAttr.set(this.projectNameDiv, "title", projectName);
        domAttr.set(this.projectNameDiv, "aria-label", projectName);
        if (domClass.contains(this.projectNameDiv, "esriCTHidden")) {
          domClass.remove(this.projectNameDiv, "esriCTHidden");
          setTimeout(lang.hitch(this, function () {
            jimuUtils.initFirstFocusNode(this.widgetDomNode, this.projectNameDiv);
            focusUtils.focus(this.projectNameDiv);
          }), 0);
        }
      }
    },
    ...
    startup: function () {
      ...
      if (this.projectInfo && this.projectInfo.name === "") {
        domAttr.set(this.projectNameDiv, "title", "");
        domAttr.set(this.projectNameDiv, "tabindex", "-1");
        if (!domClass.contains(this.projectNameDiv, "esriCTHidden")) {
          domClass.add(this.projectNameDiv, "esriCTHidden");
          setTimeout(lang.hitch(this, function () {
            jimuUtils.initFirstFocusNode(this.widgetDomNode, this.featureModeTextNode);
            focusUtils.focus(this.featureModeTextNode);
          }), 0);
        }
      }
    ...
    */
    _initProjectOverview: function () {
      var totalCostAriaLabel, grossCostAriaLabel;
    ...
    showAssetItemSummary: function (assetItemSummary, updateProjectBoundary) {
      ...
      if (Object.keys(assetItemSummary).length > 0) {
        var layerId, summaryContainer, assetTable, assetTableBody, tableHeaderRow, tableRow,
          currentTemplate, templateName, unitsValue, unitsAbbr, ariaLabelText;
        ...
        for (layerId in assetItemSummary) {
          ...
          for (templateName in assetItemSummary[layerId].templates) {
            ...
            tableRow = domConstruct.create("tr", { tabindex: "0" }, assetTableBody);
            ...
            ariaLabelText = assetItemSummary[layerId].layerName + " " + templateName + " " +
              unitsValue + " " + unitsAbbr + " " + this.config.generalSettings.currency + " " +
              this.appUtils.applyRounding(currentTemplate.cost);
            domAttr.set(tableRow, "aria-label", ariaLabelText);
        ...
        domConstruct.create("div", {
          ...
          "tabindex": "0",
          "aria-label": this.nls.workBench.noAssetAddedMsg,
          "role": "presentation"
    ...
    */
    grossCostUpdated: function (grossCost) {
      ...
      domAttr.set(this.grossCost, "aria-label", roundedGrossCost);
    ...
    _saveProjectGeometry: function (geometry) {
      ...
      this.projectLayer.applyEdits(null, [featureData], null, lang.hitch(this,
        function () {
          ...
          // while creating the project, add/update label point
          this.emit("addOrModifyLabelPoint", {
    ...
    */
    _saveProjectAttributes: function (projectTotalCost, grossCost) {
      ...
      array.forEach(this.projectLayer.fields, lang.hitch(this, function (field) {
        if (field.name.toLowerCase() === "totalassetcost") {
          featureData.attributes[field.name] = projectTotalCost;
        } else if (field.name.toLowerCase() === "grossprojectcost") {
