...
define([
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin, Evented], {
    ...
    _prevPointLayerCentroidValue: "", // to store project point layer centroid previous value
    ...
    postCreate: function () {
      ...
      this._prevPointLayerCentroidValue = "";
      ...
      this.own(on(this.pointLayerCentroidHelp, "click", lang.hitch(this, function () {
        this.appUtils.showMessage(this.nls.projectSettings.pointLayerHintText);
      })));
    ...
    getConfig: function () {
      ...
      config = {
        ...
        "pointLayerCentroid": this._prevPointLayerCentroidValue
      };
      return config;
    },
    ...
    _init: function () {
      ...
      this._populatePointLayerDropdown(operationalLayers);
    ...
    _setConfig: function () {
      ...
      // set point layer dropdown
      if (this.config.pointLayerCentroid &&
        this.pointLayerCentroidDropDown &&
        this.pointLayerCentroidDropDown.options.length > 1) {
        this.pointLayerCentroidDropDown.set("value", this.config.pointLayerCentroid);
    ...
    _attachingEvents: function () {
      ...
      this.own(on(this.costingGeometryLayerDropDown, "change", lang.hitch(this, function (value) {
        ...
        if (value !== "" && !this._costingGeometryLayerFieldOptions[value]) {
          var layer = this.layerInfosObj.getLayerInfoById(value);
      ...
      // on change event of project assets table dropdown
      this.own(on(this.pointLayerCentroidDropDown, "change", lang.hitch(this, function () {
        this._onDropdownChange(this.pointLayerCentroidDropDown, "pointLayerCentroidDropdown", "layer");
      })));
    },
    ...
    _setNewDropdownValue: function (dropdown, dropdownOf) {
      switch (dropdownOf) {
        ...
        case "projectLayer":
          ...
          break;
        case "pointLayerCentroidDropdown":
          if (this._prevPointLayerCentroidValue !== dropdown.value) {
            this.emit("onLayerSelected", {
              "lastSelectedId": this._prevPointLayerCentroidValue,
              "currentSelectedLayerId": dropdown.value
            });
            this._prevPointLayerCentroidValue = dropdown.value;
          }
          break;
        case "projectAssets":
    ...
    _restorePreviousDropdownValue: function (dropdown, dropdownOf) {
      switch (dropdownOf) {
        ...
        case "projectMultiplier":
          ...
          break;
        case "pointLayerCentroidDropdown":
          dropdown.set("value", this._prevPointLayerCentroidValue);
          break;
        default:
          break;
      }
    ...
    _onProjectLayerDropDownChange: function () {
      ...
      if (this.projectLayerDropDown.value) {
        layer = this.layerInfosObj.getLayerInfoById(this.projectLayerDropDown.value);
        if (layer && layer.layerObject) {
          layer = layer.layerObject;
      ...
      this.emit("onProjectLayerSelected", this.projectLayerDropDown.value);
    },
    ...
    /**
     * This function is used to check layer has valid field type and field name
     * @memberOf CostAnalysis/setting/ProjectSettings
     */
    _isValidFieldType: function (layer) {
      var layerFieldName;
      var validField = false;
      array.some(layer.fields, lang.hitch(this, function (field) {
        layerFieldName = field.name.toLowerCase();
        //layer must have projectid (GUID type)field
        if (layerFieldName === "projectid" && field.type === "esriFieldTypeGUID") {
          validField = true;
          return true;
        } else {
          validField = false;
    ...
    /**
     * This function is used to check whether layer is editable or not
     */
    ...
    /**
     * This function is used to populate point layer dropdown
     */
    _populatePointLayerDropdown: function (operationalLayers) {
      // push default value
      var pointLayerOptions = [];
      pointLayerOptions.push(this._defaultSelectOption);
      // push valid point layers
      this._getValidPointLayersOptions(pointLayerOptions, operationalLayers);
      this.pointLayerCentroidDropDown.set("options", lang.clone(pointLayerOptions));
      this._prevPointLayerCentroidValue = this.projectLayerDropDown.getValue();
      // enable/disable dropdown
      this._enableOrDisablePointLayerDropdown();
    },

    /**
     * This function is used to get valid point layers
     */
    _getValidPointLayersOptions: function (pointLayerOptions, operationalLayers) {
      array.forEach(operationalLayers, lang.hitch(this, function (operationalLayer) {
        // point layer is valid if following conditions are met
        // 1. has geometryType as esriGeometryPoint
        // 2. is editable
        // 3. has field projectid of type GUID
        // 4. having layerType as ArcGISFeatureLayer
        if (operationalLayer.hasOwnProperty('layerObject')) {
          var layerObject = operationalLayer.layerObject;
          if (layerObject.hasOwnProperty('geometryType')) {
            if (layerObject.geometryType === "esriGeometryPoint" &&
              this._isEditable(layerObject) &&
              this._isValidFieldType(layerObject) &&
              layerObject.type === "Feature Layer") {
              pointLayerOptions.push({
                "value": layerObject.id,
                "label": layerObject.title || layerObject.name
              });
            }
          }
        }
    ...
    /**
     * This function is used to enable or disable point layer dropdown
     */
    _enableOrDisablePointLayerDropdown: function () {
      var projectDropdownValue;
      projectDropdownValue = this.projectLayerDropDown.getValue();
      if (projectDropdownValue !== '' && projectDropdownValue !== null && projectDropdownValue !== undefined) {
        this.pointLayerCentroidDropDown.set("disabled", false);
      } else {
        this.pointLayerCentroidDropDown.set("disabled", true);
        this.pointLayerCentroidDropDown.set("value", "");
      }
    }
  });
