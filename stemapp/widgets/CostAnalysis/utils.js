...
define([
  ...
  */
  mo.applyRounding = function (value, places) {
    ...
    return jimuUtils.localizeNumber(returnValue);
  };
  ...
  */
  mo.roundProjectCostValue = function (configuredRoundingType, cost) {
    ...
    return jimuUtils.localizeNumber(temp_cost);
  };
  ...
  */
  mo.getPrjAssetTableFields = function (layer) {
    ...
    array.forEach(layer.fields, lang.hitch(this, function (field) {
      if (field.name.toLowerCase() === "assetguid") {
        prjTableFieldInfo.ASSETGUID = field.name;
      } else if (field.name.toLowerCase() === "costequation") {
        prjTableFieldInfo.COSTEQUATION = field.name;
      } else if (field.name.toLowerCase() === "scenario") {
        prjTableFieldInfo.SCENARIO = field.name;
      } else if (field.name.toLowerCase() === "templatename") {
        prjTableFieldInfo.TEMPLATENAME = field.name;
      } else if (field.name.toLowerCase() === "geographyguid") {
        prjTableFieldInfo.GEOGRAPHYGUID = field.name;
      } else if (field.name.toLowerCase() === "projectguid") {
  ...
  */
  mo.getPrjLayerFields = function (layer) {
    ...
    array.forEach(layer.fields, lang.hitch(this, function (field) {
      if (field.name.toLowerCase() === "projectname") {
        prjLayerFieldInfo.PROJECTNAME = field.name;
      } else if (field.name.toLowerCase() === "description") {
        prjLayerFieldInfo.DESCRIPTION = field.name;
      } else if (field.name.toLowerCase() === "totalassetcost") {
      ...
      } else if (field.name.toLowerCase() === "globalid") {
  ...
  */
  ...
  mo.validateEquation = function (value) {
     ...
     */
    mo.setFirstAndLastWBFocusNode = function (widgetDomNode, firstFocusNode, LastFocusNode) {
      jimuUtils.initFirstFocusNode(widgetDomNode, firstFocusNode);
      focusUtils.focus(firstFocusNode);
      jimuUtils.initLastFocusNode(widgetDomNode, LastFocusNode);
    };
  return mo;
});