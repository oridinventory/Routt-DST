...
define([
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    _createLoadProjectButtonArray: [],
    _previousOpenedPanel: null,
    ...
    onOpen: function () {
      ...
      //On widget open the first and last focus node should be maintained
      //The focus should also be maintained.
      setTimeout(lang.hitch(this, function () {
        this._openLastVisitedPanel(true);
      }), 100);
    },
    ...
    _initComponents: function () {
      ...
      setTimeout(lang.hitch(this, function () {
        this._setFirstAndLastFocusNode();
      }), 500);
    },
    ...
    _initCreateLoadPrj: function () {
      this._createOrLoadPrj = new CreateOrLoadProject({
        ...
        appUtils: appUtils,
        updateLastFocusNode: this._updateLastFocusNode,
        widgetDomNode: this.domNode,
        createLoadProjectButtonArray: this._createLoadProjectButtonArray
    ...
    _initWorkBenchPanel: function (projectInfo, assetInfo) {
      if (!this._workBench) {
        ...
        this.own(on(this._workBench, "showWorkBenchPanel", lang.hitch(this, function () {
          this._showPanel("workBench");
          var assetDetailsNode = query(".esriCTAssetItems", this.domNode);
          if (assetDetailsNode && assetDetailsNode.length > 0) {
            focusUtils.focus(assetDetailsNode[0]);
          }
        })));
        ...
        this.own(on(this._workBench, "titleClicked", lang.hitch(this, function (selectedIndex) {
          this._previousOpenedPanel = "workBench";
          if (selectedIndex === "0") {
        ...
        this.own(on(this._workBench, "showAssetDetails", lang.hitch(this, function () {
          setTimeout(lang.hitch(this, function () {
            this._showPanel("assetDetails");
            focusUtils.focus(this._workBench._assetDetails.additionalCostBtn);
          }), 100);
        })));
      ...
      this.own(on(this._workBench, "assetInfoUpdated",
        lang.hitch(this, function (assetItemSummary) {
          if (this._assetStatistics) {
            ...
            setTimeout(lang.hitch(this, function () {
              var lastNode = this._workBench.refreshButton;
              var firstNode = this._workBench.featureModeTextNode;
              if (this._workBench.downloadCSVBtn && this._workBench.downloadCSVBtn.style.display !== "none" &&
                !domClass.contains(this._workBench.downloadCSVBtn, "esriCTHidden") &&
                this._workBench._canExportProject) {
                lastNode = this._workBench.downloadCSVBtn;
              }
              if (!domClass.contains(this._workBench.projectNameDiv, "esriCTHidden")) {
                firstNode = this._workBench.projectNameDiv;
              }
              appUtils.setFirstAndLastWBFocusNode(this.domNode, firstNode,
                lastNode);
              if (this._workBench) {
                this._workBench._handle508AccessibilityForTemplatePicker();
              }
            }), 0);
          }
        })));
      ...
      // to set first and last focus node of work bench
      setTimeout(lang.hitch(this, function () {
        this._setFirstAndLastFocusNode();
      }), 500);
    },
    ...
    _initAddNewStatistics: function () {
      ...
      this.own(on(this._addNewStatisticsObj, "showAssetStatisticsPanel", lang.hitch(this, function () {
        this._previousOpenedPanel = "statisticsPanel";
        this._showPanel("assetStatistics");
      })));
    ...
    _initProjectAttribute: function (projectInfo) {
      ...
      if (!this._projectAttribute) {
        ...
        this.own(on(this._projectAttribute, "onOkButtonClicked", lang.hitch(this, function () {
          this._previousOpenedPanel = "projectAttribute";
          this._showPanel("workBench");
        })));
        ...
        this.own(on(this._projectAttribute, "onProjectInfoUpdate",
          lang.hitch(this, function (projectInfo) {
            var getLastVisitedWorkBenchPanel;
            ...
            getLastVisitedWorkBenchPanel = query(".esriCTProjectOverview", this.domNode);
            focusUtils.focus(getLastVisitedWorkBenchPanel[3]);
          })));
    ...
    _getAttributeSettingsInfo: function (projectDetails, costingData) {
      ...
      all(defArr).then(lang.hitch(this, function () {
        ...
        var lastNode = this._workBench.refreshButton;
        var firstNode = this._workBench.featureModeTextNode;
        if (this._workBench.downloadCSVBtn.style.display !== "none" &&
          !domClass.contains(this._workBench.downloadCSVBtn, "esriCTHidden")) {
          lastNode = this._workBench.downloadCSVBtn;
        }
        if (!domClass.contains(this._workBench.projectNameDiv, "esriCTHidden")) {
    ...
    */
    _updateFeatureAttributes: function (layerId, layerAttrSetting, asset, projectDetails, costingData) {
      var featureData, layer, prjLayer, layerFieldLength,
        prjLayerFieldLength, mappedFieldData;
      featureData = new Graphic();
      featureData.attributes = {};
      featureData.attributes = asset.attributes;
      layer = this.map.getLayer(layerId);
      prjLayer =
        this._layerInfosObj.getLayerInfoById(this.config.projectSettings.projectLayer).layerObject;
      array.forEach(layerAttrSetting, lang.hitch(this, function (attrSetting) {
        if (projectDetails) {
          //Get layer and project layer field length
          //If project layer field length is greater than the mapped layer field length
          //then truncate the field value
          layerFieldLength = layer.getField(attrSetting.layerField).length;
          prjLayerFieldLength = prjLayer.getField(attrSetting.projectField).length;
          mappedFieldData = projectDetails[attrSetting.projectField];
          if (layerFieldLength < prjLayerFieldLength && mappedFieldData &&
            mappedFieldData.length > 0) {
            mappedFieldData = mappedFieldData.slice(0, layerFieldLength);
          }
          asset.attributes[attrSetting.layerField] = mappedFieldData;
          featureData.attributes[attrSetting.layerField] = mappedFieldData;
        } else {
          if (attrSetting.projectField.toLowerCase() === "totalassetcost") {
            featureData.attributes[attrSetting.layerField] = costingData.totalCost;
          }
          if (attrSetting.projectField.toLowerCase() === "grossprojectcost") {
    ...
    _initAssetStatistics: function () {
      ...
      this.own(on(this._assetStatistics, "onOkButtonClicked", lang.hitch(this, function () {
        this._previousOpenedPanel = "assetStatistics";
        this._showPanel("workBench");
      })));
      this.own(on(this._assetStatistics, "onCancelButtonClicked", lang.hitch(this, function () {
        this._showPanel("workBench");
      })));
      this.own(on(this._assetStatistics.addNewStatisticsIcon, "click", lang.hitch(this, function () {
        this._addNewStatisticsObj._initAddNewStatistics();
        this._showPanel("statisticsPanel");
      })));
      this.own(on(this._assetStatistics.addNewStatisticsIcon, "keydown", lang.hitch(this, function (evt) {
     ...
     * @memberOf widgets/CostAnalysis/Widget
     */
    _showPanel: function (name, canFocus) {
      ...
      if (name === "workBench" && this._workBench) {
        this._workBench.onShow(canFocus);
        this._workBench._handle508AccessibilityForTemplatePicker();
      }
      this._openLastVisitedPanel();
    },
    ...
    /**
     * This function is used to open last visited panel on back button click of workbench
     */
    _openLastVisitedPanel: function (isWidgetOpened) {
      var getOpenedTitleName, paneName, getLastVisitedWorkBenchPanel,
        titleContainerArray, titleContainerArrayLen, projectNameCheckboxArray, projectNameCheckboxArrayLen;
      if (this._currentPanelName === "createLoadPrjPanel") {
        getOpenedTitleName = query(".esriCTItemTitleActive", this.domNode)[0];
        if (getOpenedTitleName) {
          paneName = domAttr.get(getOpenedTitleName, "paneName");
          switch (paneName) {
            case "creatProject":
              this._setFirstAndLastFocusNode();
              break;
            case "loadProject":
              this._setFirstAndLastFocusNode();
              titleContainerArray = query(".esriCTProjectOveriew", this.domNode);
              titleContainerArrayLen = titleContainerArray.length;
              if (titleContainerArrayLen === 2) {
                this._updateLastFocusNode(this._createLoadProjectButtonArray[3], this.domNode);
              }
              focusUtils.focus(this._createOrLoadPrj.projectNameSelect);
              break;
            case "updateCostEquation":
              this._setFirstAndLastFocusNode();
              projectNameCheckboxArray = query(".esriCTProjectNameCheckBox", this.domNode);
              focusUtils.focus(projectNameCheckboxArray[0]);
              var updateCostEquationButton =
                this._createOrLoadPrj._updateProjectCostWidget.updateCostEquationButton;
              if (!domClass.contains(updateCostEquationButton, "esriCTUpdateEquationBtnDisabled")) {
                this._updateLastFocusNode(updateCostEquationButton, this.domNode);
              }
              else {
                projectNameCheckboxArrayLen = projectNameCheckboxArray.length;
                this._updateLastFocusNode(projectNameCheckboxArray[projectNameCheckboxArrayLen - 1], this.domNode);
              }
      ...
      if (this._currentPanelName === "workBench") {
        this._setFirstAndLastFocusNode();
        getLastVisitedWorkBenchPanel = query(".esriCTProjectOverview", this.domNode);
        if (this._previousOpenedPanel === "assetStatistics") {
          focusUtils.focus(getLastVisitedWorkBenchPanel[2]);
        }
        if (this._previousOpenedPanel === "projectAttribute") {
          focusUtils.focus(getLastVisitedWorkBenchPanel[3]);
        }
        if (this._previousOpenedPanel === "assetDetails") {
          this._workBench._assetDetails.setFirstAndLastNode(isWidgetOpened);
          focusUtils.focus(getLastVisitedWorkBenchPanel[1]);
        }
        if (this._workBench) {
          this._workBench._handle508AccessibilityForTemplatePicker();
        }
      }
      if (this._currentPanelName === "assetStatistics") {
        this._assetStatistics.setFirstAndLastNode();
      }
      if (this._currentPanelName === "statisticsPanel") {
        var firstCheckBoxPanelNode = query('.esriCTAddLayerTableNode th', this.domNode)[0].children[0];
        if (domClass.contains(firstCheckBoxPanelNode, "jimu-state-disabled")) {
          focusUtils.focus(this._addNewStatisticsObj.addNewStatisticsObj.btnAddNode);
          jimuUtils.initFirstFocusNode(this.domNode, this._addNewStatisticsObj.addNewStatisticsObj.btnAddNode);
        }
        else {
          jimuUtils.initFirstFocusNode(this.domNode, firstCheckBoxPanelNode);
          focusUtils.focus(query('.esriCTAddLayerTableNode th', this.domNode)[0].children[0]);
        }
        jimuUtils.initLastFocusNode(this.domNode, this._addNewStatisticsObj.cancelButtonNode);
      }

      if (this._currentPanelName === "assetDetails") {
        this._workBench._assetDetails.setFirstAndLastNode(isWidgetOpened);
      }

      if (this._currentPanelName === "projectSummary") {
        this._workBench._projectSummary.setFirstAndLastNode();
      }
    },
    ...
    _setTheme: function () {
      ...
      if (this.appConfig.theme.name === "DashboardTheme") {
        ...
        domClass.add(this.copyFeatures, "jimu-main-background");
      } else {
    ...
    /**
     * This function is used to set first and last focusable dom elements
     */
    _setFirstAndLastFocusNode: function () {
      if (this._currentPanelName === "createLoadPrjPanel") {
        var titleContainerArray = query(".esriCTProjectOveriew", this.domNode),
          titleContainerArrayLen = titleContainerArray.length;
        focusUtils.focus(this._createOrLoadPrj.projectNameTextBox.textbox);
        if (titleContainerArrayLen) {
          jimuUtils.initFirstFocusNode(this.domNode, titleContainerArray[0]);
          this._updateLastFocusNode(titleContainerArray[titleContainerArrayLen - 1], this.domNode);
        }
      }
      if (this._currentPanelName === "workBench") {
        if (this._workBench) {
          ...
          if (domClass.contains(this._workBench.downloadCSVBtn, "esriCTHidden") ||
            domStyle.get(this._workBench.downloadCSVBtn, "display") === "none") {
            this._updateLastFocusNode(this._workBench.refreshButton, this.domNode);
          } else {
            this._updateLastFocusNode(this._workBench.downloadCSVBtn, this.domNode);
          }
          this._workBench._handle508AccessibilityForTemplatePicker();
        }
      }
    ...
    /**
     * This function is used to update last Focus node
     */
    _updateLastFocusNode: function (lastNodeDOM, widgetDomNode) {
      jimuUtils.initLastFocusNode(widgetDomNode, lastNodeDOM);
    }
  });
});