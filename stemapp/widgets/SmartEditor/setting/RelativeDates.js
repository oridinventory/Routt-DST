                    ...
                    if (this.selectValueRadioButton.checked) {
                        ...
                        if (relativeDatesData.chooseFromLayerInfo && relativeDatesData.chooseFromLayerInfo.selectedValue) {
                            ...
                            if (new Date(dateValue) != "Invalid Date") {
                                if (!this._isSelectedDateFieldHasTime()) {
                                    dateValue = this.filterUtils.formatDate(dateValue);
                                }
                                dateValue = new Date(dateValue);
                                relativeDatesData.dateTime = this._convertToEpoch(dateValue);
                            } else {
            ...
            */
            ...
            _dateValueOptionChange: function () {
                if (this.customRadioButton.checked) {
                    ...
                    domClass.add(this.noValidLayerContentNode, "esriCTHidden");
                }
                if (this.selectValueRadioButton.checked) {
                    if (this.layerSelector.layerChooser.getAllItems().length > 0) {
                    ...
                    domClass.add(this.customOptionContentNode, "esriCTHidden");
                        ...
                        domClass.add(this.customOptionContentNode, "esriCTHidden");
                        domClass.remove(this.noValidLayerContentNode, "esriCTHidden");
                    }
                }
            },
                    ...
                    if (field.domain && field.type === "esriFieldTypeDate") {
                        array.some(field.domain.codedValues, lang.hitch(this, function (codedDomain) {
                            if (selectedValue == codedDomain.code) {
                                var dateValue = selectedValue;
                                if (new Date(dateValue) != "Invalid Date") {
                                    if (!this._isSelectedDateFieldHasTime()) {
                                        dateValue = this.filterUtils.formatDate(dateValue);
                                    }
                                    selectedDateInfo.dateTime = this._convertToEpoch(new Date(dateValue));
                                } else {
            ...
            /**
             * This function is used to chcek if selected date field has time enable or not
             */
            _isSelectedDateFieldHasTime: function () {
                var hasTime = false;
                if (this.valueProviderFactory && this.valueProviderFactory.popupInfo &&
                    this.valueProviderFactory.popupInfo.fieldInfos) {
                    array.forEach(this.valueProviderFactory.popupInfo.fieldInfos, lang.hitch(this,
                        function (fieldInfo) {
                            if (fieldInfo.fieldName === this.fieldsDropdown.value) {
                                if ((fieldInfo.format && fieldInfo.format.time)) {
                                    hasTime = true;
                                    return;
                                }
                            }
