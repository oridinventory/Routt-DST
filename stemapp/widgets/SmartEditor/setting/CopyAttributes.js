  ...
  function (
    ...
    return declare([BaseWidgetSetting, Evented, _TemplatedMixin], {
      ...
      _createAddressPanel: function (tr) {
        this._fetchLoactorService().then(lang.hitch(this, function (response) {
          if (response !== null) {
            this._addressDijit = Address({
              nls: this.nls,
              _fieldValues: this._configuredFieldValues,
              _geocoderSettings: this._geocoderSettings,
              _validGeocoderFields: this._getValidGeocoderFields(),
      ...
      /**
       * This function is used to fetch loactor url and get updated candidate fields
       */
      _fetchLoactorService: function () {
        var def = new Deferred();
        this._loading.show();
        esriRequest({
          url: this._geocoderSettings.url,
          content: {
            f: 'json'
          },
          handleAs: 'json',
          ...
          if (response && response.candidateFields) {
            this._geocoderSettings.fields = lang.clone(response.candidateFields);
            //remove localized names as it is increasing the config and we are not using it
            array.forEach(this._geocoderSettings.fields, function (field) {
