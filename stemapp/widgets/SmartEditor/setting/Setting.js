  ...
  function (
    ...
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      postCreate: function () {
        ...
        this._initTabs();
        //create loading indicator for config
        ...
        LayerInfos.getInstance(this.map, this.map.itemInfo)
          .then(lang.hitch(this, function (operLayerInfos) {
            ...
            this.webmapSettingsRadioBtn.onStateChange = lang.hitch(this, function () {
              ...
              this._onWebMapSettingsRadioBtnChecked();
              }
            });
            //Listen for custom settings radio button change event
            this.customSettingsRadioBtn.onStateChange = lang.hitch(this, function () {
              if (this.customSettingsRadioBtn.get("checked")) {
              this._onCustomSettingsRadioBtnChecked();
              }
            });
            this.own(on(this.valuePicker, "click", lang.hitch(this, function () {
              //Check both the checkboxes if value picker check box is checked
              if (this.valuePicker.get("checked")) {
                this.enableAttributeUpdates.set('checked', true);
                this.enableAutomaticAttributeUpdates.set('checked', true);
              }
            })));
          }));
      },

      _onWebMapSettingsRadioBtnChecked: function () {
        //Get the settings from current displayed layer/tables
        this._getConfigForCurrentDisplayedLayers();
        //if web map radio button is checked populate the table with default web map settings
          this._updateLayersTable();
      },


      _onCustomSettingsRadioBtnChecked: function () {
        //if custom radio button is checked populate the table with previously configured settings
          this._updateLayersTable();
      },

      _updateLayersTable: function () {
        this._layersTable.clear();
        //Check the current traversal status to maintain the state of breadcrumb
        if (this._currentTableIds.length && this._currentTableIds.length > 0) {
          var currentConfig;
          array.some(this._configInfos, lang.hitch(this, function (info) {
            if (this._currentTableIds[this._currentTableIds.length - 1] === info.featureLayer.id) {
              currentConfig = info;
              return true;
            }
          }));
          //Get all the relationship infos to show the table
          array.forEach(currentConfig.relationshipInfos, lang.hitch(this, function (info) {
            info.layerInfo = this._jimuLayerInfos.getLayerOrTableInfoById(info.featureLayer.id);
          }));
          this._setLayersTable(currentConfig.relationshipInfos, true);
        } else {
          //if the first table is displayed then directly pass the default layer infos array
      ...
      **/
      _initTabs: function () {
        ...
        this.own(on(this.tab, "tabChanged", lang.hitch(this, function () {
          ...
          if (this.tab.getSelectedIndex() === 2) {
            this._resetHeight(this.attributeActionsTabNode);
          }
          else if (this.tab.getSelectedIndex() === 3){
            this._resetHeight(this.generalSettingTabNode);
          }
        })));
      ...
      _initLayersTable: function () {
        ...
        },
        {
          ...
          type: 'actions',
          'class': 'actions layersTableAction',
        ...
        var args = {
          fields: fields,
          selectable: false,
          autoHeight: false, //we will set the height programmatically
          style: {
            maxHeight: "calc(100% - 100px)"
          }
        };
        //Set height to view stack container
        domStyle.set(this.layerSettingTabNode.parentElement, "height", "calc(100% - 45px)");
      ...
      _initSettings: function () {
        ...
        //Set the value of radio buttons based on the stored configuration settings
        ...
        if (this.config.editor.hasOwnProperty("expandRelatedTableOnLoad")) {
          this.expandRelatedTableNode.set("checked", this.config.editor.expandRelatedTableOnLoad);
        } else {
          this.expandRelatedTableNode.set("checked", false);
        }

        if (this.config.editor.hasOwnProperty("expandMainLayerOnLoad")) {
          this.expandMainLayerNode.set("checked", this.config.editor.expandMainLayerOnLoad);
        } else {
          this.expandMainLayerNode.set("checked", true);
        }
        ...
        if (this.config.editor.hasOwnProperty("showValuePicker")) {
          this.valuePicker.set('checked', this.config.editor.showValuePicker);
        }
        else {
          this.valuePicker.set('checked', false);
        }
        ...
        if (this.config.editor.hasOwnProperty("featuresSelectionTolerance")) {
          this.featuresSelectionTolerance.set('value', this.config.editor.featuresSelectionTolerance);
        } else {
      ...
      */
      ...
      _setLayersTable: function (configuredLayerInfos, isRelatedInfo) {
        var nl = null, isAtLeastOneTable = false, rowHeight;
        if (isRelatedInfo) {
        ...
        array.forEach(configuredLayerInfos, function (configInfo) {
          //Get the default settings for each layer if honor webmap flag is true
          //This will help us in update the layer table with default values
          if (this.webmapSettingsRadioBtn.checked) {
            var layerDefaultSettings = editUtils.createDefaultConfigInfo(configInfo.layerInfo);
          }
          var tdEdit;
          //var hasMOrZ;
          ...
          var addRowResult = this._layersTable.addRow({
            ...
            edit: this.webmapSettingsRadioBtn.checked ? layerDefaultSettings._editFlag : configInfo._editFlag ? true : false,
            allowUpdateOnly: this.webmapSettingsRadioBtn.checked ? layerDefaultSettings.allowUpdateOnly : configInfo.allowUpdateOnly,
            allowUpdateOnlyHidden: configInfo.allowUpdateOnly === null ?
              false : configInfo.allowUpdateOnly,
            allowDelete: this.webmapSettingsRadioBtn.checked ? layerDefaultSettings.featureLayer.layerAllowsDelete : configInfo.allowDelete,
            ...
            disableGeometryUpdate: isRelatedInfo ? true : this.webmapSettingsRadioBtn.checked ? layerDefaultSettings.disableGeometryUpdate : configInfo.disableGeometryUpdate,
          ...
          //get the row height, this will be used while calculating the actual height of table
          rowHeight = domStyle.get(addRowResult.tr, "height");
          ...
          if (this.webmapSettingsRadioBtn.checked) {
            ...
            nl.forEach(function (node) {
              var widget = registry.getEnclosingWidget(node.childNodes[0]);
              widget.setStatus(false);
              if (layerDefaultSettings.allowDelete) {
          ...
          if (!configInfo.relationshipInfos || configInfo.relationshipInfos.length === 0) {
            ...
            if (this._isValidRelationship(configInfo.relationshipInfos)) {
        ...
        var editHeaderCheckBox = this._layersTable._getThCheckBox("edit");
        var updateHeaderCheckBox = this._layersTable._getThCheckBox("allowUpdateOnly");
        var deleteHeaderCheckBox = this._layersTable._getThCheckBox("allowDelete");
        var geometryHeaderCheckBox = this._layersTable._getThCheckBox("disableGeometryUpdate");
        var status = this.webmapSettingsRadioBtn.checked ? false : true;
        editHeaderCheckBox.setStatus(status);
        updateHeaderCheckBox.setStatus(status);
        deleteHeaderCheckBox.setStatus(status);
        geometryHeaderCheckBox.setStatus(status);
        //Calculate and set the table height
        //row height * total number of rows
        //37px for table header
        domStyle.set(this._layersTable.domNode, "height",
          ((rowHeight * configuredLayerInfos.length) + 37) + "px");
      },
      _isValidRelationship: function (relatedInfos) {
        var isValidRelationship = false;
        array.some(this._jimuLayerInfos.getLayerInfoArray(), lang.hitch(this, function (layer) {
          array.forEach(relatedInfos, lang.hitch(this, function (relatedInfo) {
          if (layer.id === relatedInfo.featureLayer.id) {
        ...
        if (!isValidRelationship) {
          array.some(this._jimuLayerInfos.getTableInfoArray(), lang.hitch(this, function (table) {
            array.forEach(relatedInfos, lang.hitch(this, function (relatedInfo) {
            if (table.id === relatedInfo.featureLayer.id) {
              isValidRelationship = true;
              return true;
            }
            }));
      ...
      _onDescriptionClick: function (tr) {
        ...
        //SimpleTable returns null value for any disabled check box
        //To check if layer added in the webmap is editable or not we will have to get the layers default settings
        if (this.webmapSettingsRadioBtn.checked) {
          var defaultSettings = editUtils.createDefaultConfigInfo(tr._configInfo.layerInfo);
        }
        if (rowData && (this.webmapSettingsRadioBtn.checked ? defaultSettings._editFlag : rowData.edit)) {
      ...
      _onEditFieldInfoClick: function (tr) {
        ...
        this._editFields = new EditFields({
          ...
          _configInfo: tr._configInfo,
          _layerName: rowData.label,
          isEditableLayer: rowData.edit !== null ? rowData.edit : tr._configInfo.layerInfo.layerObject.isEditable(),
          ...
          honorWebMapSettings: this.webmapSettingsRadioBtn.checked,
          editUtils: editUtils,
          map: this.map,
      ...
      _initEditor: function () {
        ...
        if (!this._editorObj) {
          ...
          if (this.config.editor.editDescription === undefined || this.config.editor.editDescription === null) {
            editorContent = this.nls.layersPage.title;
          }
          else {
            editorContent = this.config.editor.editDescription;
          }
          this._editorObj.set('value', this.editorXssFilter.sanitize(editorContent));
      ...
      **/
      ...
      _resetSettingsConfig: function () {
        ...
        this.config.editor.showValuePicker =
          this.valuePicker.checked === undefined ?
            false : this.valuePicker.checked;
      ...
      getConfig: function () {
        ...
        //validate features Selection tolerance settings
        if (!this.featuresSelectionTolerance.isValid()) {
          new Message({
            message: this.nls.layersPage.featuresSelectionToleranceErrorMsg
          });
          return false;
        }
        var def = new Deferred();
        ...
        this._loading.show();
        //added timeout as UI thread was getting blocked and loading indicator was not visible
        setTimeout(lang.hitch(this, function () {
        ...
        if (this.customSettingsRadioBtn.checked) {
          //first get the settings from current displayed layer/tables
          this._getConfigForCurrentDisplayedLayers();
        }
        //Validate fields table of each laye
        if (this.customSettingsRadioBtn.checked) {
          var fieldInfoStatus = this._validateFieldInfos(this._configInfos);
            if (fieldInfoStatus.layerID && !fieldInfoStatus.isValidFields) {
            var layerInstance;
            layerInstance = this._jimuLayerInfos.getLayerOrTableInfoById(fieldInfoStatus.layerID).layerObject;
              this._loading.hide();
            Message({
              message: string.substitute(this.nls.fieldsPage.noDisplayFieldWarningMsg, {
                layerName: layerInstance.name
              })
            });
            return false;
          }
        ...
        //Add "presetorder" only if attribute actions key exist
        if (this.config.attributeActionGroups) {
          this._getPresetGroupOrder();
          var presetDetails = this._validatePresetGroup();
          //If no field configured in the preset
          //Show the error message and open the attribute actions tab
          if (!presetDetails.isValid) {
              this._loading.hide();
            Message({
              message: string.substitute(this.nls.attributeActionsPage.presetNoFieldWarningMsg,
                { groupName: presetDetails.groupName })
            });
            this.tab.selectTab(this.nls.layersPage.attributeActionsTabTitle);
            return false;
          }
        }
      ...
      _validatePresetGroup: function () {
        var isValid = true, groupName = "";
        if (this.config.attributeActionGroups && this.config.attributeActionGroups.Preset) {
          for (var i = 0; i < this.config.attributeActionGroups.presetOrder.length; i++) {
            array.some(this.config.attributeActionGroups.presetOrder, lang.hitch(this,
              function (currentGroupName) {
                if (!this._hasFieldsInPresetGroup(
                  this.config.attributeActionGroups.Preset[currentGroupName])) {
                  isValid = false;
      ...
      _hasFieldsInPresetGroup: function (groupInfo) {
        var hasPreset = false;
        for (var layerId in groupInfo.appliedOn) {
          if (groupInfo.appliedOn[layerId].length > 0) {
            hasPreset = true;
            break;
          }
        }
      ...
      _getPresetGroupOrder: function () {
        this.config.attributeActionGroups.presetOrder = [];
        var rows = this._presetActionGroupTable.getRows();
        for (var i = 0; i < rows.length; i++) {
          this.config.attributeActionGroups.presetOrder.push(rows[i]._configInfo.name);
        }
      },
      ...
      _validateFieldInfos: function (configInfos) {
        var validFields = false, layerID;
        array.some(configInfos, function (currentConfig) {
          validFields = false;
          layerID = currentConfig.featureLayer.id;
          if (currentConfig.fieldInfos.length === 0) {
            validFields = true;
          }
          array.some(currentConfig.fieldInfos, function (fieldInfo, index) {
      ...
      _addTableFieldActionIcon: function (tableRow, relationshipInfo) {
        ...
        this.own(on(tableRow.tableFieldDiv, 'click',
          lang.hitch(this, function () {
            ...
            //Save the settings for current displayed layer/tables(Parent) only if custom setting
            //radio button is checked, no need to save the table settings when webmap settings radio button
            //is true as we will always load the default settings
            if (this.customSettingsRadioBtn.checked) {
              this._getConfigForCurrentDisplayedLayers();
            }
      ...
      _onSelectLocatorUrlOk: function (evt, isGeocoderBtnClicked, reFetchGeocoderFields) {
        var def = new Deferred();
        if (reFetchGeocoderFields) {
          var url = [{ url: evt }];
          evt = url;
        }
        ...
        esriRequest({
          ...
          if (response && response.candidateFields) {
            ...
            if (!isGeocoderBtnClicked && !reFetchGeocoderFields) {
      ...
      _configureGroupSmartAction: function (tr, isDelete) {
        ...
        if (isDelete) {
          ...
          setTimeout(lang.hitch(this, function () {
            var tableSADOM = query(".esriCTTableMainNode.tableBodyScroll", this.smartActionsTableNode.domNode),
        ...
        if (this.config.smartActionGroups) {
          for (var groupName in this.config.smartActionGroups) {
            if (this.config.smartActionGroups[groupName].appliedOn.hasOwnProperty(relatedLayerId)) {
              fieldValidations =
                this._getFieldValidationsFromGroup(this.config.smartActionGroups[groupName],
                  relatedLayerId, fieldValidations);
            }
          }
        }
        return fieldValidations;
      ...
      _createFiltersForLayerSelector: function () {
        ...
        featureLayerFilter = LayerChooserFromMap.createFilterByLayerType(['FeatureLayer', 'Table']);
       ...
       */
      _initAttributeActionsTable: function (tableContainerNode, isPreset, isCoordinate) {
        var attributeActionsTable, fields, rowActions = ['edit', 'delete'];
        //Add up, down actions for preset table
        if (isPreset) {
          rowActions = ['up', 'down', 'edit', 'delete'];
        }
        fields = [{
          name: 'name',
        ...
        //add coordinate source option only for coordinates table
        if(isCoordinate){
          fields.splice(2, 0, {
      ...
      _initAllAttributeActions: function () {
        ...
        this._coordinatesActionGroupTable =
          this._initAttributeActionsTable(this.coordinatesActionsTableNode, false, true);
        ...
        this._presetActionGroupTable =
          this._initAttributeActionsTable(this.presetActionsTableNode, true);
        ...
        if (table) {
          if(coordinatesSource){
            addRowResult = table.addRow({
              name: name,
              dataType: dataType,
              coordinateSourceTableValue: this.nls.coordinatesPage[coordinatesSource]
            });
          } else{
            addRowResult = table.addRow({
              name: name,
      ...
      _populateAttributeActionGroups: function () {
        ...
        //Skip preset order as it only used for the purpose of ordering
          if (actionName !== "presetOrder") {
            var attributeAction = this.config.attributeActionGroups[actionName];
            //For preset, take all the configured group name keys into an array
            //This array will be used to maintain the preset order
            if (actionName === "Preset") {
              if (!this.config.attributeActionGroups.hasOwnProperty('presetOrder')) {
                this.config.attributeActionGroups.presetOrder = Object.keys(attributeAction);
              }
              for (var i = 0; i < this.config.attributeActionGroups.presetOrder.length; i++) {
                var groupName = this.config.attributeActionGroups.presetOrder[i];
      ...
      _configureAddressActionGroup: function (tr, isDelete) {
        this._onSelectLocatorUrlOk(this._configuredGeocoderSettings.url, false, true).then(lang.hitch(this,
          function (response) {
            if (response !== null) {
              var params;
              params = {
      ...
      _configureCoordinatesActionGroup: function (tr, isDelete) {
        ...
        params = {
          ...
          _fieldValues: {
            "Coordinates": {
              ...
              "field": "x",
              "enabled": true,
              "coordinatesSource": "featureLocation"
            }
          },
          editUtils: editUtils
        };
        ...
        this.own(on(coordinatesObj, "groupInfoUpdated", lang.hitch(this, function (groupInfo) {
          if (!tr) {
            ...
            tr._configInfo = groupInfo;
            var coordinateSource= groupInfo.attributeInfo &&
            typeof (groupInfo.attributeInfo.coordinatesSource) !== "undefined" ?
            groupInfo.attributeInfo.coordinatesSource : "featureLocation";
            this._coordinatesActionGroupTable.editRow(tr, {
              ...
              coordinateSourceTableValue: this.nls.coordinatesPage[coordinateSource]
            });
          }
          //store intersection attributeActions Group
          this.config.attributeActionGroups = this._getAttributeActionGroupConfig();
      ...
      _configurePresetActionGroup: function (tr, isDelete) {
        ...
        if (tr) {
          ...
          params.chooseFromLayerInfo = tr._configInfo.chooseFromLayerInfo;
        }
      ...
      _filterAttributeDropDownOptions: function (currentRow) {
        var rows, configuredDataType, coordinatesOption;
        //by default add all the option for feature location and my location
        coordinatesOption = {
          "featureLocation": {},
          "myLocation": {}
        };
        ...
        if (rows && rows.length > 0) {
          array.forEach(rows, lang.hitch(this, function (tr) {
            var index, coordinateSource = "featureLocation";
            ...
            if (!currentRow || !(
              ...
              if (coordinatesOption[coordinateSource].MapSpatialReference.indexOf(configuredDataType) > -1) {
                ...
                coordinatesOption[coordinateSource].MapSpatialReference.splice(index, 1);
              } else if (coordinatesOption[coordinateSource].LatLong.indexOf(configuredDataType) > -1) {
                index = coordinatesOption[coordinateSource].LatLong.indexOf(configuredDataType);
                coordinatesOption[coordinateSource].LatLong.splice(index, 1);
              } else if (coordinatesOption[coordinateSource].MGRS.indexOf(configuredDataType) > -1) {
                coordinatesOption[coordinateSource].MGRS.splice(0, 1);
              }
            }
          }));
        }
      ...
      _resetHeight: function (targetTabPanel) {
