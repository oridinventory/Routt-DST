    ...
    return declare([_WidgetBase, Evented, _TemplatedMixin], {
          ...
          this._processSelectedFeatures(selectedFeatures, processDeferred).then(lang.hitch(this, function () {
            ...
            for (var layerId in this.featureTitlesByLayerId) {
              if (this.featureTitlesByLayerId.hasOwnProperty(layerId) && matchedLayerId.indexOf(layerId) === -1 &&
                (this.layerInfosObj.getLayerInfoById(layerId) || this.map._layers.hasOwnProperty(layerId))) {
      ...
      _processSelectedFeatures: function (selectedFeatures, processDeferred) {
        ...
        array.forEach(selectedFeatures, lang.hitch(this, function (feature) {
          ...
          if (this.layerInfosObj.getLayerInfoById(feature._layer.id)) {
            ...
            //for graphic layer features if attributes are available then use first attribute value as feature title
            var featureTitle = feature.attributes && Object.keys(feature.attributes).length > 0 ?
              feature.attributes[Object.keys(feature.attributes)[0]] : this.nls.copyFeatures.geometryLabel;
          }
        ...
        for (featureLayerId in this.featuresByLayerId) {
          if (featureLayerId && this.layerInfosObj.getLayerInfoById(featureLayerId)) {
            ...
            deferredObj[featureLayerId] = this.featuresByLayerId[featureLayerId];
          }
        }
        all(deferredObj).then(lang.hitch(this, function (deferredObjDetails) {
          for (layerId in deferredObjDetails) {
            if (layerId && this.layerInfosObj.getLayerInfoById(layerId)) {
      ...
      */
      _fieldMatchingBtnClicked: function () {
        ...
        for (var layerId in this.featuresByLayerId) {
          if (this.featuresByLayerId.hasOwnProperty(layerId) && matchedLayerId.indexOf(layerId) === -1 &&
            ...
            matchedLayerId.push(layerId);
            sourceLayersInfo.push(this._getSourceLayerInfo(layerId));
          }
        }
        //create source layer info for the graphic layers
        for (var layerId in this.featuresByLayerId) {
          if (this.featuresByLayerId.hasOwnProperty(layerId) && matchedLayerId.indexOf(layerId) === -1 &&
            this.map._layers.hasOwnProperty(layerId)) {
            sourceLayersInfo.push(this._createGraphicLayerInfo(layerId));
          }
        }
      ...
      _createFeatureListUI: function (currentLayerId, geometryType) {
        ...
        layer = this.layerInfosObj.getLayerInfoById(layerId) || this.map._layers[currentLayerId];
        //Get the layer title
        layerName = layer.title ? layer.title : layer.name;
        if (!layerName) {
          layerName = this.useLayerIdAsLayerName(layerId);
        }       
      ...
      /**
       * This function is used to create source layer info for graphic layer
       */
      _createGraphicLayerInfo: function (currentLayerId) {
        var layer, layerName, fields, layerId;
        layerId = currentLayerId;
        //Get layer info by id
        layer = this.map._layers[layerId];
        fields = layer.graphics && layer.graphics.length > 0 && layer.graphics[0].attributes &&
          Object.keys(layer.graphics[0].attributes).length > 0 ?
          this._createGraphicLayerFields(layer.graphics[0]) : [];
        if (layer && fields) {
          //Get the layer title
          layerName = layer.title ? layer.title : layer.name;
          if (!layerName) {
      ...
      useLayerIdAsLayerName: function(layerId){
        var layerName;
        //if directions widgets layer then display title from nls else use layerId
        if (layerId.includes("directions_routeLayer_esri_dijit_Directions")) {
          layerName = this.nls.copyFeatures.directionsRouteLabel;
        } else if (layerId.includes("directions_waypointsEventLayer_esri_dijit_Directions")) {
          layerName = this.nls.copyFeatures.directionsWaypointsLabel;
        } else if (layerId.includes("directions_stopLayer_esri_dijit_Directions")) {
          layerName = this.nls.copyFeatures.diretionsStopLabel;
        } else {
          layerName = layerId;
        }
        return layerName;
