    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin, Evented], {
      ...
      startup: function () {
        ...
        this.gridOutlineColorPicker =
          new ColorPickerEditor({
              nls: this.nls,
              typeOfColorPicker: "Outline"
            },
            this.cellOutlineColorPicker);
        ...
        this.gridFillColorPicker = new ColorPickerEditor({
            nls: this.nls,
            typeOfColorPicker: "Fill"
          },
          this.cellFillColorPicker);
        ...
        this.colorPickerNodes = query('.colorPickerEditor', this.domNode);
      ...
      _loadOptionsForDropDown: function (dropDown, dropDownOptions) {
        ...
        array.forEach(dropDownOptions, lang.hitch(this, function (type) {
          if (this.nls.gridSettings[type] && this.nls.gridSettings[type].hasOwnProperty("label")) {
            option = {
              value: type,
              label: this.nls.gridSettings[type].label
            };
          } else {
            option = {
              value: type,
              label: this.nls.gridSettings[type]
            };
          }
          options.push(option);
        }));
      ...
      /**
       * validate colorPicker spinners
       **/
      validInputs: function () {
        var isValid = true;
        //validate for any invalid values in all colorPicker spinners
        array.some(this.colorPickerNodes, function (node) {
          if (dijitRegistry.byId(node.id).validateSpinner && !dijitRegistry.byId(node.id).validateSpinner()) {
            isValid = false;
            return true;
          }
        }, this);
