    ...
    return declare([BaseWidget, dijitWidgetBase, dijitWidgetsInTemplate], {
      ...
      postCreate: function () {
        ...
        if (!projection.isSupported()) {
          new Message({
            message: this.nls.projectionNotSupported
          });
        } else {
          projection.load();
        }
        ...
        if (window.isRTL && kernel.locale === "he") {
          domStyle.set(this.helpBtnForArea, "transform", "rotateY(0deg)");
          domStyle.set(this.helpBtnForPt, "transform", "rotateY(0deg)");
        } else {
          domStyle.set(this.helpBtnForArea, "transform", "");
          domStyle.set(this.helpBtnForPt, "transform", "");
        }

        this._setDijitErrorMsg();
        ...
        this.own(on(this.grgPointBySizeCoordTool, 'coordinates-deleted', lang.hitch(this, function () {
          this._toggleGrgPointCreateGRGButton();
        })));

        this.own(on(this.grgPointBySizeCoordTool, 'get-coordinate-complete', lang.hitch(this, function () {
          this._toggleGrgPointCreateGRGButton();
        })));
        ...
        this.own(on(this.grgPointByRefSystemCoordTool, 'coordinates-deleted', lang.hitch(this, function () {
          this._toggleGrgPointCreateGRGButton();
        })));

        this.own(on(this.grgPointByRefSystemCoordTool, 'get-coordinate-complete', lang.hitch(this, function () {
          this._toggleGrgPointCreateGRGButton();
        })));
        ...
        this.own(on(this.grgPointByTimeCoordTool, 'coordinates-deleted', lang.hitch(this, function () {
          this._toggleGrgPointCreateGRGButton();
        })));

        this.own(on(this.grgPointByTimeCoordTool, 'get-coordinate-complete', lang.hitch(this, function () {
          this._toggleGrgPointCreateGRGButton();
        })));
      ...
      startup: function () {
        ...
        //Change the label format based on the current selected coordinate system
        if (this._referenceSystem === "UTM") {
      ...
      resize: function () {
        ...
        if (nl.length > 0) {
          domStyle.set(nl[0], "width", (this.grgAreaBySizePublishGRGButton.clientWidth - 44) + "px");
        }
      },
      ...
      _grgAreaBySizePublishGRGButtonClicked: function () {
        var params = {
          name: (this.publishNewLayer.checked) ? this.addGRGNameArea.value : this.featureLayerList.getValue(),
          url: "",
          itemId: "",
          serviceItemId: ""
        };
        if (this.publishNewLayer.checked) {
          if (this._isValidGRGLayerName(params.name)) {
            this.publishMessage.innerHTML = '';
          } else {
            // Invalid entry
      ...
      _handleClickEvents: function () {
        ...
        this.own(on(this.grgAreaByRefSystemGridSize, 'change', lang.hitch(this, function () {
          //this function will update the label format based on grid area/point
          this._updateLabelFormat(this.grgAreaByRefSystemGridSize, this.grgAreaByRefLabelFormat);
        })));
        ...
        **/
        ...
        this.own(on(this.grgPointBySizeCoordTool.dt, 'DrawComplete', lang.hitch(this, function () {
          this._dt_Complete();
          this._onDrawCompleteToggleCreateGRGButton();
        ...
        this.own(on(this.grgPointByRefSystemGridSize, 'change', lang.hitch(this, function () {
          //this function will update the label format based on grid area/point
          this._updateLabelFormat(this.grgPointByRefSystemGridSize, this.grgPointByRefLabelFormat);
          this._toggleGrgPointCreateGRGButton();
        })));
        ...
        this.own(on(this.gridSettingsPanelBackButton, "click", lang.hitch(this, function () {
          if (this._gridSettingsInstance.validInputs()) {
        ...
        //Handles the resize of the select elements
      ...
      _createGridSettings: function () {
        ...
        this.own(this._gridSettingsInstance.on("gridSettingsChanged",
          lang.hitch(this, function (updatedSettings) {
            ...
            this.GRGArea.refresh();
            //Update the label format settings on change of reference system
            this._updateLabelFormat(this.grgAreaByRefSystemGridSize, this.grgAreaByRefLabelFormat);
            this._updateLabelFormat(this.grgPointByRefSystemGridSize, this.grgPointByRefLabelFormat);
          })));
      ...
      _showPanel: function (currentPanel) {
        ...
        this._supportAccessibility();
        if (currentPanel === 'publishPage' && !this.isOnScreen) {
          this.resize();
        }
      },
      ...
      _grgAreaBySizeCreateGRGButtonClicked: function () {
        var geoSpatialReference = new SpatialReference(4326);
        ...
        if (this._graphicsLayerGRGExtent.graphics[0] && this.grgAreaBySizeCellWidth.isValid() &&
          ...
          //project geometry in 4326
          var projectedGeometry = (projection.isLoaded()) ?
            projection.project(this._graphicsLayerGRGExtent.graphics[0].geometry, geoSpatialReference) :
            this._graphicsLayerGRGExtent.graphics[0].geometry;
          //based on rotation get geometry or its extent
          var geom = (this.angle === 0) ?
            gridGeomUtils.extentToPolygon(projectedGeometry.getExtent()) : projectedGeometry;
          ...
          if (drawGRG.checkGridSize(numCellsHorizontal, numCellsVertical)) {
            //get center point from the geometry
      ...
      _grgPointBySizeCreateGRGButtonClicked: function () {
        if (domClass.contains(this.grgPointBySizeCreateGRGButton, "jimu-state-disabled")) {
          return;
        }
        //check form inputs for validity
      ...
      _grgPointByTimeCreateGRGButtonClicked: function () {
        if (domClass.contains(this.grgPointByTimeCreateGRGButton, "jimu-state-disabled")) {
          return;
        }
      ...
      _grgPointByRefSystemCreateGRGButtonClicked: function () {
        if (domClass.contains(this.grgPointByRefSystemCreateGRGButton, "jimu-state-disabled")) {
          return;
        }
          ...
          if (this.grgPointByRefCellHorizontal.isValid() &&
            ...
            switch (this.grgPointByRefSystemGridSize.getValue()) {
              case 'UTM':
                for (var i = 0; i < zones.length; i++) {
                  if (projection.isLoaded()) {
                    graphic = new Graphic(projection.project(zones[i].gridPolygon.unclippedPolygon,
                  ...
                  //Check for the current selected reference system and accordingly
                  //update the label based on label format
                  if (this._referenceSystem === "UTM") {
                    var utmCoordinate = mgrs.LLtoUTM(
                      zones[i].nonPolarGridZone.extent.ymin,
                      zones[i].nonPolarGridZone.extent.xmin,
                      zones[i].utmZone)[3];
                    //remove the extra spaces
                    utmCoordinate.s = utmCoordinate.s.trim();
                    //set the band in the label if configured
                    label = label.replace(/S/, utmCoordinate.s[utmCoordinate.s.length - 1]);
                    //set the zone number in the label if configured
                    label = label.replace(/Z/, utmCoordinate.z);
                  } else {
              ...
              default:
                ...
                for (i = 0; i < polysToLoop.length; i++) {
                  if (projection.isLoaded()) {
                    graphic = new Graphic(projection.project(polysToLoop[i].clippedPolyToUTMZone,
                      this.map.spatialReference));
                  } else {
                  ...
                  if (this._labelTypeWithRefSys === 'gridReferenceSystem') {
                    ...
                    if (this._referenceSystem === "UTM") {
                      var projectedExtent =
                        projection.project(graphic.geometry.getExtent(),
                          new SpatialReference(4326));
                      var coordinateValue =
                        mgrs.LLtoUTM(projectedExtent.ymin,
                          projectedExtent.xmin, polysToLoop[i].utmZone)[3];
                      coordinateValue.s = coordinateValue.s.trim();
                      label = label.replace(/Y/, coordinateValue.y);
                      label = label.replace(/X/, coordinateValue.x);
                      label = label.replace(/S/, coordinateValue.s[coordinateValue.s.length - 1]);
                      label = label.replace(/Z/, coordinateValue.z);
                    } else {
            ...
            // This step fixes https://github.com/ArcGIS/solutions-defense-widgets/issues/189
            // Zipper cells affected the retrieval of zone polygons. This function essentially
            // removes the extraneous polygons when creating 5x5 grid from a reference system (for example)
            features = this._trimProcessZonesPolygons(features, this.grgPointByRefCellHorizontal.getValue(),
              this.grgPointByRefCellVertical.getValue());
      ...
      /**
       * Removes unneeded zones from feature list
       */
      _trimProcessZonesPolygons: function (graphics, width, height) {

        function getNextGraphic(startGraphic, isRight) {
          var retGraphic = null;
          for (var i = 0; i < graphics.length; i++) {
            // Exclude the bottom left polygon
            if (!GeometryEngine.equals(startGraphic.geometry, graphics[i].geometry)) {
              // Check if polygons intersect
              if (GeometryEngine.intersects(startGraphic.geometry, graphics[i].geometry)) {
                // Get the angle between start and intersected graphic. Since we're trying
                // to find the graphic to the right of the start graphic, it has to be close
                // to 90 degrees
                var angle = geometryUtils.getRotationAngleBetweenPoints(startGraphic.geometry.getCentroid(),
                  graphics[i].geometry.getCentroid());
                if (isRight) {
                  if (angle >= 80 && angle <= 100) {
                    retGraphic = graphics[i];
                    break;
                  }
                } else {
                  // Since we're trying to find the graphic up from the start graphic, it has to be close
                  // to 0 degrees
        ...
        // First get the bottom left start poly
        var startGraphic = graphics[0]; //It's always the first index
        // Search for graphics to the right
        var nextGraphic = getNextGraphic(startGraphic, true);
        var i = 1, graphicsToDelete = [];
        while (nextGraphic) {
          // If graphics go beyond the width, keep track of
          // these graphics to delete
          if (i === width) {
            graphicsToDelete.push(nextGraphic);
          }
          nextGraphic = getNextGraphic(nextGraphic, true); i++;
        }

        // Search for graphics to the top using the items in the deleted graphic list
        i = 1;
        array.forEach(graphicsToDelete, function (deleteGraphic) {
          nextGraphic = getNextGraphic(deleteGraphic, false);
          while (nextGraphic) {
            graphicsToDelete.push(nextGraphic);
            nextGraphic = getNextGraphic(nextGraphic, false); i++;
          }
        }, this);

        // Search for graphics to the top using the start graphic
        nextGraphic = getNextGraphic(startGraphic, false);
        i = 1; var stoppedIdx = graphicsToDelete.length;
        while (nextGraphic) {
          // If graphics go beyond the width, keep track of
          // these graphics to delete
          if (i === height) {
        ...
        // Iterate through the delete list and search for graphics to the right.
        // These zones to the right shouls also be deleted
        if (graphicsToDelete.length > stoppedIdx) {
          array.forEach(graphicsToDelete.slice(stoppedIdx, graphicsToDelete.length), function (deleteGraphic) {
            nextGraphic = getNextGraphic(deleteGraphic, true);
            while (nextGraphic) {
              graphicsToDelete.push(nextGraphic);
              nextGraphic = getNextGraphic(nextGraphic, true);
            }
          }, this);
        }

        // Trim the graphics list using the delete list
        var indexList = [];
        for (var j = graphics.length - 1; j >= 0; j--) {
          for (var k = 0; k < graphicsToDelete.length; k++) {
            if (graphics[j] && graphics[j].hasOwnProperty("geometry") &&
             graphicsToDelete[k] && graphicsToDelete[k].hasOwnProperty("geometry")) {
              if (GeometryEngine.equals(graphics[j].geometry, graphicsToDelete[k].geometry)) {
                indexList.push(j);
              }
            }
          }
      ...
      _grgAreaByRefSystemCreateGRGButtonClicked: function () {
        ...
        //check form inputs for validity
        if (this._graphicsLayerGRGExtent.graphics[0]) {
          var geographicGeometry = (projection.isLoaded()) ?
            projection.project(this._graphicsLayerGRGExtent.graphics[0].geometry, new SpatialReference(4326)) :
            this._graphicsLayerGRGExtent.graphics[0].geometry;
          ...
          if (drawGRG.checkGridSize(numCellsHorizontal, numCellsVertical) ||
            ...
            var zones = mgrsUtils.zonesFromExtent(geographicGeometry.getExtent(), this.map);
            var features = [];
            ...
            switch (this.grgAreaByRefSystemGridSize.getValue()) {
              case 'UTM':
                for (i = 0; i < zones.length; i++) {
                  if (this.grgAreaByRefSystemClipToggle.checked) {
                    graphic = new Graphic((projection.isLoaded()) ?
                      projection.project(zones[i].gridPolygon.clippedPolygon,
                        this.map.spatialReference) : zones[i].gridPolygon.clippedPolygon);
                  } else {
                    graphic = new Graphic((projection.isLoaded()) ?
                      projection.project(zones[i].gridPolygon.unclippedPolygon,
                        this.map.spatialReference) : zones[i].gridPolygon.unclippedPolygon);
                  }
                  geomArray.push(graphic.geometry);
                  ...
                  if (this._referenceSystem === "UTM") {
                    ...
                    //set the band in the label if configured
                    label = label.replace(/S/, utmCoordinate.s[utmCoordinate.s.length - 1]);
                    //set the zone in the label if configured
                    label = label.replace(/Z/, utmCoordinate.z);
                  } else {
                    label = label.replace(/Z/, zones[i].gridPolygon.text);
                  }
                ...
                break;
              default:
                var polysToLoop = mgrsUtils.processZonePolygons(zones, this.map, geographicGeometry.getExtent());
                var currentValue = 10000;
                while (currentValue >= this.grgAreaByRefSystemGridSize.getValue()) {
                  ...
                  for (i = 0; i < polysToLoop.length; i++) {
                    polys = polys.concat(mgrsUtils.handleGridSquares(polysToLoop[i], this.map,
                      currentValue, geographicGeometry.getExtent()));
                  }
                  polysToLoop = [];
                  polysToLoop = polys;
                  currentValue = currentValue / 10;
                ...
                for (var i = 0; i < polysToLoop.length; i++) {
                  if (this.grgAreaByRefSystemClipToggle.checked) {
                    if (projection.isLoaded()) {
                      graphic = new Graphic(projection.project(polysToLoop[i].clippedPolygon,
                        this.map.spatialReference));
                    } else {
                      graphic = new Graphic((this.map.spatialReference.wkid !== 4326) ?
                        polysToLoop[i].clippedPolygon :
                        WebMercatorUtils.webMercatorToGeographic(polysToLoop[i].clippedPolygon)
                      );
                    }
                    geomArray.push(graphic.geometry);
                  } else {
                    if (projection.isLoaded()) {
                      ...
                      graphic = new Graphic((this.map.spatialReference.wkid !== 4326) ?
                        polysToLoop[i].clippedPolyToUTMZone :
                        WebMercatorUtils.webMercatorToGeographic(polysToLoop[i].clippedPolyToUTMZone)
                      );
                    }
                    geomArray.push(graphic.geometry);
                  }
                    ...
                    if (this._referenceSystem === "UTM") {
                      var projectedExtent =
                        projection.project(graphic.geometry.getExtent(),
                          new SpatialReference(4326));
                      var coordinateValue =
                        mgrs.LLtoUTM(projectedExtent.ymin, projectedExtent.xmin,
                          polysToLoop[i].utmZone)[3];
                      coordinateValue.s = coordinateValue.s.trim();
                      label = label.replace(/Y/, coordinateValue.y);
                      label = label.replace(/X/, coordinateValue.x);
                      label = label.replace(/S/, coordinateValue.s[coordinateValue.s.length - 1]);
      ...
      _initSaveToPortal: function (params) {
        var featureServiceName = params.name;
        // sign in
        ...
        portal.signIn().then(lang.hitch(this, function (portalUser) {
          ...
          if (portalUser.role === "org_user") {
            this.publishMessage.innerHTML = utils.sanitizeHTML(this.nls.createService.format(this.nls.userRole));
            this._updateLastFocusNode();
            return;
          }
          ...
          if (!params.url) {
            ...
            // Check if user supplied name is available
                ...
                drawGRG.createFeatureService(createServiceUrl, token,
                  ...
                  if (response1.success) {
                    ...
                    drawGRG.addDefinitionToService(addToDefinitionUrl, token,
                      ...
                      this.busyIndicator.hide();
                      this.publishMessage.innerHTML =
                        utils.sanitizeHTML(this.nls.addToDefinition.format(err2.message));
                      this._updateLastFocusNode();
                    }));
                  } else {
                    ...
                    this.busyIndicator.hide();
                    this.publishMessage.innerHTML =
                      utils.sanitizeHTML(this.nls.unableToCreate.format(featureServiceName));
                    this._updateLastFocusNode();
                  }
                }), lang.hitch(this, function (err1) {
                  // Error in calling create feature service REST call
                  this.busyIndicator.hide();
                  this.publishMessage.innerHTML = utils.sanitizeHTML(this.nls.createService.format(err1.message));
                  this._updateLastFocusNode();
                }));
              } else {
      ...
      /**
       * This function is used to enable or disable GrgPoint CreateGRGButton
       *  Once needed values are specified
       */
      _toggleGrgPointCreateGRGButton: function () {
        if (this._currentOpenPanel === "grgPointBySize") {
          if (this.grgPointBySizeCoordTool.getMapCoordinateDD()) {
            domClass.remove(this.grgPointBySizeCreateGRGButton, "jimu-state-disabled");
            domAttr.set(this.grgPointBySizeCreateGRGButton, "tabindex", 0);
          } else {
            domClass.add(this.grgPointBySizeCreateGRGButton, "jimu-state-disabled");
            domAttr.set(this.grgPointBySizeCreateGRGButton, "tabindex", -1);
          }
        }

        if (this._currentOpenPanel === "grgPointByRefSystem") {
          if (this.grgPointByRefSystemCoordTool.getMapCoordinateDD()) {
            domClass.remove(this.grgPointByRefSystemCreateGRGButton, "jimu-state-disabled");
            domAttr.set(this.grgPointByRefSystemCreateGRGButton, "tabindex", 0);
          } else {
            domClass.add(this.grgPointByRefSystemCreateGRGButton, "jimu-state-disabled");
            domAttr.set(this.grgPointByRefSystemCreateGRGButton, "tabindex", -1);
          }
        }

        if (this._currentOpenPanel === "grgPointByTime") {
          if (this.grgPointByTimeCoordTool.getMapCoordinateDD()) {
            domClass.remove(this.grgPointByTimeCreateGRGButton, "jimu-state-disabled");
            domAttr.set(this.grgPointByTimeCreateGRGButton, "tabindex", 0);
          } else {
            domClass.add(this.grgPointByTimeCreateGRGButton, "jimu-state-disabled");
            domAttr.set(this.grgPointByTimeCreateGRGButton, "tabindex", -1);
          }
        }
      },

      /**
       * This function is used to toggle create grd button on drawComplete
       */
      _onDrawCompleteToggleCreateGRGButton: function () {
        setTimeout(lang.hitch(this, function () {
          this._toggleGrgPointCreateGRGButton();
        }), 500);
      },
      ...
      /**
       * this function will update the label format based on grid type
       */
      _updateLabelFormat: function (selectNode, label) {
        switch (selectNode.getValue()) {
          case 'UTM':
            label.value = this._referenceSystem === "UTM" ? 'ZS' : 'Z';
            break;
          case '100000':
            label.value = this._referenceSystem === "UTM" ? 'ZSXY' : 'ZS';
            break;
          default:
            label.value = 'ZSXY';
            break;
      ...
      /**
       * This function is used to set error msg for dijits
       */
      _setDijitErrorMsg: function () {

        this.grgAreaBySizeRotation.invalidMessage = this.nls.invalidNumberMessage;
        this.grgAreaBySizeRotation.rangeMessage = this.nls.gridAngleInvalidRangeMessage;

        this.gridAnglePoint.invalidMessage = this.nls.invalidNumberMessage;
        this.gridAnglePoint.rangeMessage = this.nls.gridAngleInvalidRangeMessage;

        this.grgPointByTimeRotation.invalidMessage = this.nls.invalidNumberMessage;
        this.grgPointByTimeRotation.rangeMessage = this.nls.gridAngleInvalidRangeMessage;

        this.grgPointByTimeRotation.invalidMessage = this.nls.invalidNumberMessage;
        this.grgPointByTimeRotation.rangeMessage = this.nls.gridAngleInvalidRangeMessage;

        this.addGRGNameArea.missingMessage = this.nls.missingLayerNameMessage;

        var dijitAttachPointArray = [
          this.cellHorizontal, this.cellVertical, this.grgAreaBySizeCellWidth, this.grgAreaBySizeCellHeight,
          this.grgPointBySizeCellHorizontal, this.grgPointBySizeCellVertical, this.grgPointBySizeCellWidth,
          this.grgPointBySizeCellHeight, this.grgPointByRefCellHorizontal, this.grgPointByRefCellVertical,
          this.grgPointByTimeTime, this.grgPointByTimeRate, this.grgPointByTimeCellHorizontal,
          this.grgPointByTimeCellVertical, this.grgPointByTimeCellWidth, this.grgPointByTimeCellHeight
        ];

        array.forEach(dijitAttachPointArray, lang.hitch(this, function (node) {
          node.invalidMessage = this.nls.invalidNumberMessage;
          node.rangeMessage = this.nls.invalidRangeMessage;
        }));
      }
    });
  });