...
function(
    ...
    exportUtils, ViewStack, SymbolChooser, DrawBox, Message, jimuUtils, jimuSymbolUtils,SimpleRenderer,FeatureLayer
) {
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
        ...
        listGenerateDrawTable: function() {
			//remove all operational layers if exists
			for(lyr in this.oplayers){
					this.map.removeLayer(this.oplayers[lyr]);
					delete this.oplayers[lyr];
			}
            ...
            for (var i = nb_graphics - 1; i >= 0; i--) {
				...
				//create layers from graphics
				if(this.CreateOPlayers.checked && symbol.type != "textsymbol"){
					//create renderer
					var renderer = new SimpleRenderer(symbol);
					renderer=renderer.toJson();
					//create feature
					feat=new Graphic(graphic.toJson());
					feat.attributes["objectid"]=1;
					feat.symbol=undefined; //remove graphic symbol
					//geomtype
					geometrytype=feat.geometry.type.charAt(0).toUpperCase() + feat.geometry.type.slice(1);
					//create featurelayer
					var featureCollection = {
					  "layerDefinition": null,
					  "featureSet": {
						"features": [feat.toJson()],
					...
					layername=feat.attributes["name"]
					if(layername in this.oplayers){
						layername=layername+"_"+i;
					};
					//create a feature layer based on the feature collection
					this.oplayers[layername] = new FeatureLayer(featureCollection, {
					  id: layername,
					  //infoTemplate: popupTemplate
					});
					this.map.addLayer(this.oplayers[layername]);
				};
			};
			//replace drawing layer at top
			if(this.CreateOPlayers.checked){
				this.map.removeLayer(this.drawBox.drawLayer);
				this.map.addLayer(this.drawBox.drawLayer);
			};
        ...
        editorOnClickEditSaveButon: function() {
            ...
            if (this.editorSymbolChooser.type != "text") {
				...
				if(this.showMeasure.checked){ //hack romain : empecher etiquetage distances si cache non cochée
					this.addMeasure(geom, this._editorConfig["graphicCurrent"]);
				} //fin hack
            }
        ...
        postCreate: function() {
			...
			//Init Op Layers
			this.oplayers={};
			on(this.CreateOPlayers,"change",lang.hitch(this,this.listGenerateDrawTable));
